...
export default function DealsPage() {
  ...
  const filtered = deals.filter(d => {
    ...
    if (search) {
      ...
      return d.inputs.clientName.toLowerCase().includes(q)||d.inputs.salesperson.toLowerCase().includes(q)||d.id.includes(q)
    }
    return true
  })
  ...
  const selectAll = () => setSelectedIds(selectedIds.size===filtered.length?new Set():new Set(filtered.map(d=>d.id)))
    ...
    <div className="min-h-screen bg-cream-100">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-8">
        ...
        <div className="bg-cream-50 border border-cream-300 rounded-2xl p-4 mb-5 flex flex-wrap gap-3 items-center">
          <input className="hugo-input w-52" placeholder="Search client / salesperson…" value={search} onChange={e=>setSearch(e.target.value)} />
              ...
              <table className="w-full" style={{tableLayout:'fixed',minWidth:'900px'}}>
                <thead>
                  <tr className="border-b border-cream-300 bg-cream-100">
                    <th className="px-4 py-3 w-8"><input type="checkbox" className="rounded" checked={selectedIds.size===filtered.length&&filtered.length>0} onChange={selectAll}/></th>
                    {['Client','Salesperson','Geography','Floor Rate','Proposed','Floor GM','Prop. GM','Inv. Case','Status','Submitted',''].map(h=>(
                      <th key={h} className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-hugo-muted">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((deal,i)=>(
                    ...
                    <tr key={deal.id} className={`deal-row ${i%2===0?'bg-cream-50':'bg-cream-100/50'}`}>
                      <td className="px-4 py-3"><input type="checkbox" className="rounded" checked={selectedIds.has(deal.id)} onChange={()=>toggleSelect(deal.id)}/></td>
                      ...
                      <td className="px-4 py-3 text-sm text-hugo-black">{deal.inputs.salesperson}</td>
                      ...
                      <td className="px-4 py-3 text-[11px] text-hugo-muted">
                      ...
                      <tr key={`${deal.id}-exp`}>
                        <td colSpan={12} className="px-6 py-4 bg-cream-200/60 border-b border-cream-300">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[11px]">
                            <div>
                              <div className="font-bold text-hugo-muted mb-1 text-[9px] uppercase tracking-widest">Deal details</div>
                            ...
                            <div>
                              <div className="font-bold text-hugo-muted mb-1 text-[9px] uppercase tracking-widest">Team</div>
                              ...
                              <div>Shift: {deal.inputs.shiftType}</div>
                            </div>
                            <div>
                              <div className="font-bold text-hugo-muted mb-1 text-[9px] uppercase tracking-widest">Revenue (proposed)</div>
                              ...
                              <div>Annual GP: {fmt(deal.proposedPL.annualGP,'usd',0)}</div>
                            </div>
                            <div>
                              <div className="font-bold text-hugo-muted mb-1 text-[9px] uppercase tracking-widest">Review</div>
        ...
        <div className="fixed inset-0 bg-hugo-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-cream-50 border border-cream-300 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="p-6 border-b border-cream-300">
              ...
              <p className="text-[12px] text-hugo-muted mt-1">{reviewingDeal.inputs.clientName} · {reviewingDeal.inputs.geography}</p>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  {label:'Proposed GM', val:fmt(reviewingDeal.proposedPL.grossMarginPct,'pct'), cls:reviewingDeal.proposedPL.investmentCaseRequired?'text-red-600':'text-emerald-700'},
                ].map(({label,val,cls})=>(
              ...
              {reviewingDeal.investmentCaseNotes && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                  <div className="text-[10px] font-bold text-amber-600 mb-1">Investment Case Justification</div>
                  <div className="text-[11px] text-amber-700">{reviewingDeal.investmentCaseNotes}</div>
                </div>
              )}
              <div>
