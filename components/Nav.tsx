    ...
    <nav className="bg-cream-100 border-b border-cream-300 sticky top-0 z-40">
      <div className="max-w-screen-2xl mx-auto px-6">
        <div className="flex items-center justify-between h-14">
          ...
          {/* Logo — gold hugo text, no black square */}
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <Link key={href} href={href}
                className={`px-4 py-2 rounded-chip text-[12px] font-medium transition-all ${
                  active(href) ? 'bg-hugo-black text-cream-100' : 'text-hugo-muted hover:text-hugo-black hover:bg-cream-200'
                }`}>
                {label}
              </Link>
            ))}
            ...
            <div className="relative ml-1">
              <button onClick={() => setOpen(!open)}
                className="px-3 py-2 rounded-chip text-[12px] font-medium transition-all flex items-center gap-2 text-hugo-muted hover:text-hugo-black hover:bg-cream-200">
                {user?.photoURL ? (
                ...
                <div className="absolute right-0 top-full mt-2 w-52 bg-cream-50 border border-cream-300 rounded-xl shadow-lg z-50 overflow-hidden">
                  ...
                  {isAdmin(user?.email) && (
                    <Link href="/settings" onClick={() => setOpen(false)}
                      className="flex items-center px-4 py-3 text-[12px] text-hugo-black hover:bg-cream-200 transition-colors">
                      Assumptions
                    </Link>
                  )}
                  <div className="border-t border-cream-300" />
                  <button onClick={handleSignOut}
                    className="flex items-center px-4 py-3 text-[12px] text-red-500 hover:bg-red-50 transition-colors w-full text-left">
                    Sign out
                  </button>
                </div>
              )}
